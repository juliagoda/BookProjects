...
void GitQlientRepo::updateCache()
{
   ...
   {
       QLog_Debug("UI", QString("The repository directory is empty"));
   }
}
...
{
    QLog_Info("UI", QString("The attempt of setting repo: {%1}").arg(newDir));

   if (!newDir.isEmpty() || (newDir != "No repo"))
   {
...
void GitQlientRepo::createProgressDialog()
{
   ...
   {
       QLog_Debug("UI", QString("Loading repository couldn't be initialized"));
   }
}
